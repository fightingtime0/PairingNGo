...
// --- 2. Floor and cap ------------------------------------------------------
{
  ...
  assert.strictEqual(swiss.winPct({ wins: 5, losses: 0, draws: 0, roundsPlayed: 5 }), 0.75);
  assert.ok(Math.abs(swiss.winPct({ wins: 3, losses: 2, draws: 0, roundsPlayed: 5 }) - 0.6) < 1e-9);
  console.log('floor 25% / cap 75% / 3-2 = 60% all correct');
}
...
for (const n of sizes) {
  for (let seed = 1; seed <= 5; seed++) {
    ...
    for (const row of standings) {
      assert.ok(row.opWinPct >= 0.25 - 1e-9 && row.opWinPct <= 0.75 + 1e-9, 'OWP out of band');
      assert.ok(row.opOpWinPct >= 0.25 - 1e-9 && row.opOpWinPct <= 0.75 + 1e-9, 'OOWP out of band');
    }
  }
}
