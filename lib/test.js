...
// --- 6. Top cut seeding ----------------------------------------------------
{
  ...
  // A bracket round that is confirmed but not signed off must not advance.
  settle(cut.rounds[0]);
  assert.throws(() => swiss.advanceCut(cut), /Confirm bracket round 1/,
    'an unlocked bracket round must not advance');
  cut.rounds[0].locked = true;

  swiss.advanceCut(cut);
  assert.strictEqual(cut.rounds[1].matches.length, 2);
  settle(cut.rounds[1]);
  cut.rounds[1].locked = true;
  swiss.advanceCut(cut);
  assert.strictEqual(cut.rounds[2].matches.length, 1);
  console.log('top 8 bracket advances 4 -> 2 -> 1, and only past a confirmed round');
}

// --- 7. Ties as a double loss ---------------------------------------------
{
  // The same reported result, scored two ways. Nothing about what the players
  // reported changes — only what the event says it is worth.
  const base = () => ({
    players: [
      { id: 'p1', name: 'A', seed: 1 },
      { id: 'p2', name: 'B', seed: 2 },
    ...
    rounds: [{
      number: 1,
      locked: true,
      matches: [{ id: 'm1', table: 1, p1: 'p1', p2: 'p2', bye: false, result: 'draw', status: 'confirmed' }],
    }],
  });

  const asDraw = base();
  asDraw.settings = { tieMode: 'draw' };
  const drawRecs = swiss.buildRecords(asDraw);
  assert.strictEqual(drawRecs.get('p1').points, 1, 'a draw is worth a point');
  assert.strictEqual(drawRecs.get('p1').draws, 1);
  assert.strictEqual(drawRecs.get('p1').losses, 0);

  const asDouble = base();
  asDouble.settings = { tieMode: 'doubleLoss' };
  const dlRecs = swiss.buildRecords(asDouble);
  assert.strictEqual(dlRecs.get('p1').points, 0, 'a double loss is worth nothing');
  assert.strictEqual(dlRecs.get('p2').points, 0, 'to either player');
  assert.strictEqual(dlRecs.get('p1').draws, 0, 'and records no draw');
  assert.strictEqual(dlRecs.get('p1').losses, 1, 'it records a loss');

  // Win% has no draws to halve, so it reads straight off the losses.
  assert.strictEqual(swiss.winPct(dlRecs.get('p1'), 'official'), 0.25, 'floored at 25%');
  assert.strictEqual(swiss.winPct(dlRecs.get('p1'), 'turni'), 0, 'turni mode has no floor');

  // An event with no tieMode set behaves exactly as it always did.
  const legacy = base();
  assert.strictEqual(swiss.tieModeOf(legacy), 'draw', 'missing tieMode must default to draw');
  assert.strictEqual(swiss.buildRecords(legacy).get('p1').points, 1);
  console.log('tie mode: draw = 1 pt each, doubleLoss = 0 pts and a loss each, default unchanged');
}

// --- 8. turni.id compatibility mode --------------------------------------
{
