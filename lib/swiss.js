 ...
 *
 * Opponent win % = (wins + 0.5 * draws) / rounds played, floored at 0.25,
 * capped at 0.75. Rounds where a player received a bye are excluded entirely,
...
/** A single player's own win percentage, bounded. Byes already excluded. */
function winPct(record) {
  if (!record || record.roundsPlayed === 0) return WIN_PCT_FLOOR;
  const raw = (record.wins + 0.5 * record.draws) / record.roundsPlayed;
  return Math.min(WIN_PCT_CAP, Math.max(WIN_PCT_FLOOR, raw));
}

function averageOf(values) {
  if (values.length === 0) return WIN_PCT_FLOOR;
...
function computeStandings(tournament) {
  ...
  for (const [id, rec] of records) ownPct.set(id, winPct(rec));

  const opWinPct = new Map();
  for (const [id, rec] of records) {
    opWinPct.set(id, averageOf(rec.opponents.map((oid) => ownPct.get(oid) ?? WIN_PCT_FLOOR)));
  }

  const opOpWinPct = new Map();
  for (const [id, rec] of records) {
    opOpWinPct.set(id, averageOf(rec.opponents.map((oid) => opWinPct.get(oid) ?? WIN_PCT_FLOOR)));
  }
  ...
  rows.sort((a, b) => {
    ...
    if (a.arrivedLate !== b.arrivedLate) return a.arrivedLate ? 1 : -1;
