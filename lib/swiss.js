 ...
 *
 * Ties have two settings. In the default 'draw' mode a tie is worth a point to
 * each player and counts as a draw in the record. In 'doubleLoss' mode a tie is
 * worth nothing to either player and counts as a loss for both — which is how
 * turni.id's event #277 ended up with ten Double Loss boards and no way back.
 * Modelling it as a legitimate, configurable outcome is the point: the result
 * is still stored as 'draw', only its scoring changes, so switching the setting
 * never rewrites what was actually reported at the table.
...
/** 'draw' (default) or 'doubleLoss'. Read from settings, never assumed. */
function tieModeOf(tournament) {
  const mode = tournament && tournament.settings && tournament.settings.tieMode;
  return mode === 'doubleLoss' ? 'doubleLoss' : 'draw';
}
...
function buildRecords(tournament) {
  ...
  const doubleLoss = tieModeOf(tournament) === 'doubleLoss';
  ...
  for (const round of tournament.rounds) {
    for (const match of round.matches) {
      ...
      if (match.result === 'draw' && doubleLoss) {
        // Both players lose. No draw is recorded, so win% needs no special case.
        a.losses += 1;
        b.losses += 1;
...
function pairNextRound(tournament, rng) {
  ...
  return {
    ...
    matches,
    hadRematch,
    // The round is not signed off yet. Nothing advances past it until the
    // organizer locks it, so a half-entered round can never draw the next one.
    locked: false,
    lockedAt: null,
    timer: null,
...
function buildCut(tournament, size) {
  ...
  return {
    ...
    rounds: [{ number: 1, matches, locked: false, lockedAt: null, timer: null, startedAt: new Date().toISOString() }],
  };
}

function advanceCut(cut, opts) {
  ...
  if (!last.locked && !force) {
    throw new Error(`Confirm bracket round ${last.number} before advancing.`);
  }
